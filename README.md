...
在 Worker 的 `Settings` -> `Variables` 中可以配置以下环境变量；未配置时使用表中的默认值：

| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `ASSET_URL` | `https://hunshcn.github.io/gh-proxy/` | 静态资源 URL |
| `PREFIX` | `/` | 路由前缀；例如路由为 `example.com/gh/*` 时填写 `/gh/` |
| `JSDELIVR_ENABLED` | `false` | 是否使用 jsDelivr 镜像；`1`、`true`、`yes`、`on` 表示开启 |
| `WHITE_LIST` | `[]` | JSON 字符串数组；目标 URL 包含任一字符串时才允许代理 |
| `WHITELIST_RULES_URL` | 空 | 白名单规则源码 URL；每行格式为 `owner/repo` |
| `BLACKLIST_RULES_URL` | 项目默认黑名单源码 | 黑名单规则源码 URL；设为空字符串可禁用远程黑名单 |
| `RULES_REFRESH_INTERVAL` | `300` | 远程规则刷新间隔（秒，最小 60 秒） |
| `JSDELIVR_BASE_URL` | `https://cdn.jsdelivr.net/gh` | jsDelivr 镜像基础 URL |

例如 `WHITE_LIST` 可填写 `["/owner/", "/another-owner/repo/"]`。设置非空白名单后，不匹配的请求会返回 403。

远程规则支持 `owner/repo`、`owner` 和 `*/repo`，空行及以 `#` 开头的行会被忽略。Worker 会在请求到达且缓存过期时刷新；也可配置 Cloudflare Cron Trigger，由 `scheduled` 事件主动预热规则缓存。拉取失败时继续使用上一次成功的规则，避免临时网络故障清空访问控制。
