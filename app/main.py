...
try:
    ...
    from security import sanitize_request_headers
...
ASSET_URL = os.getenv('ASSET_URL', 'https://benzbrake.github.io/gh-proxy').rstrip('/')
...
ENV_CONNECT_TIMEOUT = env_float('CONNECT_TIMEOUT', 5, minimum=0.1)
ENV_READ_TIMEOUT = env_float('READ_TIMEOUT', 30, minimum=0.1)
REQUEST_TIMEOUT = (ENV_CONNECT_TIMEOUT, ENV_READ_TIMEOUT)
...
try:
    index_html = requests.get(ASSET_URL, timeout=REQUEST_TIMEOUT).text
    icon_r = requests.get(
        ASSET_URL + '/favicon.ico', timeout=REQUEST_TIMEOUT).content
    forbidden_html = requests.get(
        ASSET_URL + '/forbidden.html', timeout=REQUEST_TIMEOUT).text
    logging.info('Static resources cached from %s', ASSET_URL)
except requests.RequestException as exc:
    logging.warning('Failed to fetch static resources from %s: %s', ASSET_URL, exc)
...
def iter_limited_content(response, size_limit, chunk_size=CHUNK_SIZE):
    """Stream at most size_limit bytes and always release the upstream response."""
    transferred = 0
    try:
        for chunk in iter_content(response, chunk_size=chunk_size):
            transferred += len(chunk)
            if transferred > size_limit:
                raise ResponseSizeLimitExceeded(
                    f'upstream response exceeds {size_limit} bytes')
            yield chunk
    finally:
        response.close()
...
def proxy(u, allow_redirects=False):
    ...
    try:
        ...
        def generate():
            try:
                for chunk in iter_limited_content(r, ENV_SIZE_LIMIT):
                    yield chunk
            except ResponseSizeLimitExceeded as exc:
                logging.warning('proxy response aborted: %s url=%s', exc, url)
                raise
